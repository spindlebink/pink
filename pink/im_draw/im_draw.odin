...
@(init, private)
_module_init :: proc() {
	canvas._core.hooks.imd_init = init
	canvas._core.hooks.imd_destroy = destroy
	canvas._core.hooks.imd_flush = flush
}
...
flush :: proc(pass: ^render.Pass) {
	...
	render.pass_set_binding_uniform(pass, 0, _core.ubuf)
	...
	for cmd, i in _core.cmds {
		...
		case Draw_Solid_Command:
			render.pass_set_pipeline(pass, _core.solid_pipeline)
			render.pass_set_buffers(pass, _core.solid_vbuf, _core.solid_ibuf)
			
			switch cmd.cmd.(Draw_Solid_Command).type {
			case .Rect:
				render.pass_draw(pass, 0, 6, current_solid, cmd.times)
			}
		...
		case Draw_Image_Command:
			...
			render.pass_set_pipeline(pass, _core.image_pipeline)
			render.pass_set_buffers(pass, _core.image_vbuf, _core.image_ibuf)
			render.pass_set_binding_texture(pass, 1, tex)

			// if tex._fmt == .Gray && in_rgba_mode {
			// 	render.pass_set_push_constants(pass, Image_Pipeline_Push_Constants{
			// 		rgba_convert = true,
			// 	})
			// 	in_rgba_mode = false
			...
			render.pass_draw(pass, 0, 6, current_image, cmd.times)
